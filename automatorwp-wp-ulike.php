...
final class AutomatorWP_WP_Ulike_Integration {
    ...
    private function constants() {
        // Plugin version
        define( 'AUTOMATORWP_WP_ULIKE_VER', '1.0.2' );
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - WP Ulike requires %s and %s in order to work. Please install and activate them.', 'automatorwp-wp-ulike-integration' ),
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - WP Ulike Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-wp-ulike-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_WP_ULIKE_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_wp_ulike_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-wp-ulike-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-wp-ulike-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-wp-ulike-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-wp-ulike-integration/ folder
            load_textdomain( 'automatorwp-wp-ulike-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-wp-ulike-integration/languages/ folder
            load_textdomain( 'automatorwp-wp-ulike-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-wp-ulike-integration', false, $lang_dir );
        }
